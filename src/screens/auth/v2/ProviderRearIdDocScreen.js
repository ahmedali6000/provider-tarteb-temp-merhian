...
const ProviderRearIdDocScreen = ({navigation, route}) => {
  ...
  const cropImageToScanFrame = async originalUri => {
    ...
    console.log('REAR ID CROP DEBUG:', {
      imageWidth,
      imageHeight,
      previewWidth,
      previewHeight,
  ...
  const takePhoto = async () => {
    ...
    try {
      ...
      console.log('TAKE REAR ID PHOTO ERROR:', error);
  ...
  const handleConfirm = async () => {
    if (!capturedImage?.file) {
      setErrorText(
        tr(
          'provider_register.rear_id_required',
          'من فضلك التقط صورة ظهر البطاقة',
        ),
      );
      return;
    }
    ...
    try {
      await completeProviderStep({
        step: 'rear_id_doc',
        phone: phoneNumber,
        rearIdDoc: capturedImage.file,
      });

      Alert.alert(
        tr('common.success', 'تم بنجاح'),
        tr(
          'provider_register.registration_completed',
          'تم إرسال بياناتك للمراجعة بنجاح',
        ),
        [
          {
            text: tr('common.ok', 'حسنًا'),
            onPress: () => {
              navigation.reset({
                index: 0,
                routes: [
      ...
      <>
        ...
        <View style={styles.instructionsCard}>
          ...
          <View style={styles.instructionRow}>
            <Ionicons name="checkmark" size={15} color="#3498db" />
            <AppText style={styles.instructionText}>
              {tr('provider_register.id_rule_2', 'تأكد من وضوح البيانات')}
            </AppText>
          </View>
          ...
          <View style={[styles.instructionRow, styles.lastInstructionRow]}>
            <Ionicons name="checkmark" size={15} color="#3498db" />
            <AppText style={styles.instructionText}>
              {tr('provider_register.id_rule_3', 'تجنب الظلال المباشر')}
            </AppText>
          </View>
        </View>
      </>
    ...
    return (
      <View style={styles.headerContainer}>
        ...
        <AppText style={styles.subtitle}>
          {tr(
            'provider_register.verify_identity_back_subtitle',
            'قم بتصوير الوجه الخلفي لبطاقتك الشخصية للتأكد من هويتك وضمان أمان الخدمة.',
          )}
        </AppText>
        ...
        <AppButton
          title={
            takingPhoto
              ...
              : tr(
                  'provider_register.capture_back_id_photo',
                  'التقاط صورة البطاقة',
                )
          }
          onPress={takePhoto}
  ...
  const renderReviewView = () => {
    return (
      <View style={styles.reviewBody}>
        ...
        {!!capturedImage?.sizeKB && (
          <AppText style={styles.imageSizeText}>{capturedImage.sizeKB} KB</AppText>
        )}
        ...
        <View style={styles.reviewButtonsRow}>
          <TouchableOpacity
            ...
            <AppText weight="bold" style={styles.confirmButtonText}>
              {tr(
                'provider_register.confirm_finish_registration',
...
export default ProviderRearIdDocScreen; 